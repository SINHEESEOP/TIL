# 📊 데이터 구조 (Data Structure)

다양한 데이터 구조의 개념, 특징, 구현 방법을 정리한 문서입니다.

## 📚 주요 내용

### 기본 개념
- [자료구조_개념서](./자료구조_개념서.md) - 자료구조의 근본과 이해

### 해시 기반 자료구조
- [Hash](./Hash.md) - 해시 용어 설명 및 개념
  - [OpenAddressing](./OpenAddressing.md) - 개방 주소법과 탐색 전략
- [LinkedHashMap](./LinkedHashMap.md) - LinkedHashMap 구현 및 개념, 순서 유지 특성 정리

### 집합 자료구조
- [Set](./Set.md) - 집합(Set)의 개념과 구현체 비교 (HashSet, TreeSet 등)

### 트리 기반 자료구조
- [Tree](./Tree.md) - 트리(Tree) 및 관련 파생 개념 정리
- [AdvancedTree](./AdvancedTree.md) - 고급 트리 (AVL, Red-Black, B-Tree 등)

## 🔍 학습 목표
- 각 데이터 구조의 특징과 사용 사례에 대한 이해
- 시간 복잡도와 공간 복잡도 분석
- 자바를 이용한 데이터 구조 구현 및 최적화
- 실제 문제 해결에 적합한 데이터 구조 선택 능력 향상 